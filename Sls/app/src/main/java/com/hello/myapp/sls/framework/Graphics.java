 ...
 * Created by Cloud on 2017-07-25.
    ...
    디스크로부터 이미지를 로드해  나중에 그릴 수 잇게 메모리에 저장한다.
    지난 번 프레임으로부터 남은 내용을 지울 수 있게 색상을 사용해 프레임버퍼를 정리한다.
    특정 위치의 프레임 버퍼 픽셀을 특정색상으로 설정한다.
    프레임버퍼에 선과 사각형을 그린다.
    앞에서 로드한 이미지를 프레임버퍼에 그린다. 이때 전체 이미지를 그리거나 이미지의 일부영역을 그린다. 아울러 블렌딩을 활성화 또는 비활성화해 이미지를 그릴 수 있어야한다.
    프레임 버퍼의 크기를 알아온다.
